        ...
        if ($result2 && $result2->num_rows > 0) {
            ...
            if ($result3 && $result3->num_rows > 0) {
                ...
                if (mail($to, $subject, $message, $header)) {
                    $sql = "INSERT INTO booking (tid, roomno) VALUES ('" . $row2['tid'] . "', '" . $row1['roomno'] . "')";  // Use 'bookings' table instead of 'booking_requests'
                    
                    if ($conn->query($sql) === true) {
                        // Delete the booking request
                        $sqls = "DELETE FROM booking_requests WHERE bid ='$bid' ";
